...
internal static class Verifier
{
    public static Expression<Action<Microsoft.Extensions.Logging.ILogger>> GetVerifierExpression(
        LoggerVerifyOptions loggerVerifyOptions, Mock<Microsoft.Extensions.Logging.ILogger> loggerMockObject)
    {
        return logger => logger.Log(
            ...
            It.Is<It.IsAnyType>((@object, @type) => MatchHelper.MatchText(@object, loggerVerifyOptions, loggerMockObject)),
            It.Is<Exception>(e => MatchHelper.MatchException(e, loggerVerifyOptions)),
            It.IsAny<Func<It.IsAnyType, Exception?, string>>());
    }
    
    public static Expression<Action<ILogger<T>>> GetGenericVerifierExpression<T>(LoggerVerifyOptions loggerVerifyOptions, Mock<ILogger<T>> loggerMock)
    {
        return logger => logger.Log(
            It.Is<LogLevel>(l => MatchHelper.MatchLogLevel(l, loggerVerifyOptions)),
            It.Is<EventId>(e => MatchHelper.MatchEventId(e, loggerVerifyOptions)),
            It.Is<It.IsAnyType>((@object, @type) => MatchHelper.MatchText(@object, loggerVerifyOptions, loggerMock)),
            It.Is<Exception>(e => MatchHelper.MatchException(e, loggerVerifyOptions)),
            It.IsAny<Func<It.IsAnyType, Exception?, string>>());
    }
}